...
def main():
    ...
    try:
        ...
        _ =config.get('kostal', 'url')
        _ =config.get('kostal', 'username')
        _ =config.get('kostal', 'password')
        _ =config.getint('kostal', 'interval')
        _ =config.get('kostal', 'location')
    ...
    try:
        ...
        api_response, connection_ok=getpikodata(config)
    ...
    try:
        ...
        if "401" in str(e):
            logging.error("Failed to connect to device '%s' using credentials. Check username and password!" %
    ...
    while running:
        ...
        api_response, connection_ok=getpikodata(config)
        json_string, state_flag=raw2json(api_response)
        #print(json_string)
        MQTT_msg=convert_to_mqtt_msg(json_string,config)
